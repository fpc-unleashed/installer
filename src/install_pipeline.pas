...
const
  ...
  STAGE_NAME: array[TInstallStage] of string = (
    'init', 'bootstrap FPC 3.2.2', 'fpc-unleashed source', 'building native FPC',
    'building utils', 'installing FPC', 'fpc.cfg', 'building i386 cross compiler',
    'building wasm cross compiler', 'building x86_64-linux cross compiler', 'building i386-linux cross compiler', 'lazarus source',
    'building lazbuild + LCL', 'fetching addon components', 'registering Lazarus packages', 'building Lazarus IDE',
    'writing IDE config', 'desktop shortcut', 'done');
...
{$ifdef LINUX}
  ...
  if FindFirst(HostFpcBinDir+'ppc*', faAnyFile, SR) = 0 then try
    repeat
      if (SR.Name <> '.') and (SR.Name <> '..') and
         ((SR.Attr and faDirectory) = 0) then RunSilent('/bin/ln', ['-sf', '../lib/fpc/'+Ver+'/'+SR.Name, HostFpcUtilDir+SR.Name]);
    until FindNext(SR) <> 0;
  finally
    FindClose(SR);
  end;
...
begin
  ...
  var Common: TStringArray := [
    'OS_TARGET=win64', 'CPU_TARGET=x86_64', 'OS_SOURCE='+HostTargetOs, 'CPU_SOURCE=x86_64',
    'FPCDIR='+MakeWorkDir, 'FPCFPMAKE='+PpHost, 'INSTALL_PREFIX='+FpcInstallPrefix, 'CROSSOPT=-Xi',
    'CROSSINSTALL=1'
  ];
...
begin
  ...
  var FpcInstallPrefix := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc';
  var Common: TStringArray := [
    'OS_TARGET=win32', 'CPU_TARGET=i386', 'OS_SOURCE='+HostTargetOs, 'CPU_SOURCE=x86_64',
    'FPCDIR='+MakeWorkDir, 'FPCFPMAKE='+PpHost, 'INSTALL_PREFIX='+FpcInstallPrefix, 'CROSSOPT=-Xi',
    'CROSSINSTALL=1'
  ];
...
begin
  ...
  var FpcInstallPrefix := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc';
  var Common: TStringArray := [
    'OS_TARGET=linux', 'CPU_TARGET=i386', 'OS_SOURCE='+HostTargetOs, 'CPU_SOURCE=x86_64',
    'FPCDIR='+MakeWorkDir, 'FPCFPMAKE='+PpHost, 'INSTALL_PREFIX='+FpcInstallPrefix, 'CROSSOPT=-Xi',
    'CROSSINSTALL=1'
  ];
...
{$endif}
    ...
    if FCfg.InstallLazarus and (not hasLazExe) then begin
      ...
      var addonsChanged := (FCfg.InstallMinimap <> Manifest.InstallMinimap) or
                           (FCfg.InstallCPUView <> Manifest.InstallCPUView) or
                           (FCfg.InstallMetaDarkStyle <> Manifest.InstallMetaDarkStyle)
{$ifdef MSWINDOWS}
