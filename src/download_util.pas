...
begin
  ...
  try
    var Connection := InternetOpenUrl(Session, PChar(URL), nil, 0, INTERNET_FLAG_NO_UI or INTERNET_FLAG_RELOAD or
    ...
    try
      ...
      var LastReportTotal: Int64 := 0;
       if Assigned(OnProgress) then begin if ContentLength > 0 then
...
begin
  ...
  var Code := RunSilent('curl', ['-fsSL', '-o', DestPath, URL]);
   ...
   if Code = -1 then begin if Assigned(OnProgress) then OnProgress(-1, 'curl not found in PATH -- install via your package manager (e.g. apt install curl)');
    Exit;
  end;
   if Code <> 0 then begin if Assigned(OnProgress) then OnProgress(-1, Format('curl exit=%d (URL: %s)', [Code, URL]));
    Exit;
  end;
