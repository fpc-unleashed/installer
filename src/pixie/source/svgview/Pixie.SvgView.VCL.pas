unit Pixie.SvgView.VCL;

// TPixieSvgView — Delphi VCL SVG display component.
// Renders SVG via Direct2D/DirectWrite, scales proportionally.

interface

uses
  SysUtils, Classes, Windows, Messages, Math,
...
type
  ...
  TPixieSvgView = class(TCustomControl)
  private
...
function ColorToWebColor(AColor: TColor): TPixieWebColor;
var
  Rgb: LongInt;
begin
...
function TPixieSvgView.GetScaleFactor: Single;
var
  F: TCustomForm;
begin
...
procedure TPixieSvgView.WMEraseBkgnd(var Msg: TWMEraseBkgnd);
begin
  Msg.Result := 1;
end;
...
begin
  ...
  try
    ...
    if Color = clNone then
      BgColor := ColorToWebColor(clBtnFace)
    else
      BgColor := ColorToWebColor(Color);
    ...
    begin
      InnerColor := ColorToWebColor(FColorInner);
      Inner2Color := ColorToWebColor(FColorCheckers);
    ...
    begin
      BorderColor := ColorToWebColor(FColorInnerBorder);
