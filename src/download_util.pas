...
// download URL to DestPath; Windows uses WinINet, Linux shells out to curl
...
begin
  ...
  try
    ...
    try
      // codeload uses chunked transfer-encoding and skips Content-Length; we fall back to "X MB downloaded"
...
begin
  ...
  // -fsSL: fail on >=400, silent meter but surface errors, follow GitHub release redirects;
  // no per-byte progress -- parsing curl's \r-overwritten stderr isn't worth it for ~70MB downloads
