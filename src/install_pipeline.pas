...
type
  ...
  TInstallConfig = record
    ...
    CrossWasm:      Boolean;
    // optional Lazarus IDE addons. Each is independent; toggling one off means
    // we skip --add-package for it. lazbuild --build-ide picks up whatever's
    // in <pcp>/staticpackages.inc and links it via the {$IFDEF AddStaticPkgs}
    // block in lazarus.pp, so no source patching is needed -- the lazarus
    // packagesystem.pas:2533 sets that define automatically when building the IDE.
    InstallMinimap: Boolean;
    // CPU-View IDE plugin (instructions/registers/stack views) with its
    // FWHexView runtime dependency. Both packages travel together as a single
    // user-facing checkbox; pipeline registers FWHexView.LCL + FWHexView_D.LCL
    // + the host-platform CPUView_<plat>_D.lpk.
    InstallCPUView: Boolean;
    // ToggleDisplayAffinity IDE plugin: adds a Window menu entry that flips
    // SetWindowDisplayAffinity on the IDE main window, so screen-capture /
    // screen-share tools omit the editor. The plugin source gates its actual
    // logic with {$ifdef WINDOWS}; on Linux it compiles to an empty Register
    // procedure. The UI surfaces it as a disabled checkbox on Linux hosts to
    // make the platform restriction obvious.
    InstallToggleAffinity: Boolean;
    // user-side preference; pipeline only persists it to manifest so the next
    // install run can restore the checkbox state.
    LaunchAfter:    Boolean;
    // SHA we resolved at the UI layer (head of chosen branch or user-provided
    // hash). pipeline writes this into the manifest at end of install so a
    // later run can decide whether to refresh.
    FpcSelectedSha: string;
    LazSelectedSha: string;
    // when True, pipeline appends every Log() line to <TargetDir>\installer.log
    // (truncated at start of each run).
    SaveLog: Boolean;
  end;
  ...
  // pipeline stages, ordered. each gets a slice of the 0..100 overall bar via
  // STAGE_END below. boundaries are tuned to typical wall-clock proportions:
  // build steps dominate, downloads + cfg are short.
  TInstallStage = (
    isInit,
    ...
    isFpcCfg,           // 48..49   fpcmkcfg (must run before any cross step so Linux cross can patch fpc.cfg)
  ...
  private
    ...
    // optional installer.log writer; nil when save-log is off. owned and
    // freed inside Execute so lifetime is exactly the pipeline run.
...
const
{$ifdef MSWINDOWS}
  // portable extract of fpc-3.2.2.i386-win32.exe (Inno Setup), unpacked off-line
  // to skip registry, PATH, file association, shortcut side effects.
  // ppc386.exe (FPC 3.2.2, i386-win32 native) bootstraps the unleashed compiler.
  // Earlier we tried switching to the win32+win64 combo bundle and bootstrapping
  // with ppcrossx64.exe -- on this host's FPC version that breaks `make all`
  // for the native target with 'Cannot open include file x86_64.inc'. The
  // i386-only bootstrap is the one the diagnostic scripts proved to work end-to-end.
...
{$endif}
{$ifdef LINUX}
  // portable extract of the upstream fpc-3.2.2.x86_64-linux.tar (the cross-distro
  // tarball; install.sh-style prefix, not .deb/.rpm). Repacked as ZIP so the
  // same TUnZipper path handles both host OSes; Linux unix exec bits are lost
  // during zipping, the pipeline must restore +x on bin/* and lib/fpc/3.2.2/ppc*
  // after extract. Layout inside the zip (no wrapper dir): bin/, lib/, man/, share/.
  BOOTSTRAP_URL = 'https://github.com/fpc-unleashed/freepascal/releases/download/bootstrappers-v1/fpc-3.2.2-x86_64-linux-portable.zip';
  BOOTSTRAP_SHA = 'C6072EE3E47DB6280E16347BAF4B2637B3A09FC2AB8D14B50F69C06579F55B02';
{$endif}
...
{$endif}
  ...
  // on Linux because they end up concatenated with TargetDir which is in the
  // platform's native separator already (TSelectDirectoryDialog returns either form).
{$ifdef MSWINDOWS}
  ExeExt              = '.exe';
...
{$endif}
  ...
  // Cross-toolchain mirrors hosted on the FPC bootstrap release. _BIN:
  // cross-binutils (Win32 PE producing Linux ELF). _LIB: glibc runtime +
  // Ubuntu 18.04 shared objects for full LCL widget-set.
  ...
  // Optional Lazarus IDE add-on packages distributed in a separate components-v1
  // release. Two upstream projects shipped here:
  //   FWHexView 2.0.16 -- cross-platform HEX viewer (MIT)
  //   CPUView 1.0      -- debugger plugin (MIT), depends on FWHexView.LCL
  // Each download lands as a zip in TargetDir/components-extra/ and is extracted
  // in place. Sources only -- lazbuild rebuilds .lpk into .ppu against the
  // freshly-built host RTL.
  COMPONENTS_FWHEX_URL   = 'https://github.com/fpc-unleashed/freepascal/releases/download/components-v1/FWHexView_2.0.16.zip';
  COMPONENTS_FWHEX_SHA   = 'B6CDF3A768811F557AA17C45A6F321FEBF0419B3280FA6EEE3BBBE2BB515D3F8';
  COMPONENTS_CPUVIEW_URL = 'https://github.com/fpc-unleashed/freepascal/releases/download/components-v1/CPUView_1.0.zip';
  COMPONENTS_CPUVIEW_SHA = '07455CDF621EA75F98431A7F9FB7A44CD4D9AE188288C1A0A27FD8A642BBB931';
  ...
  COMPONENTS_TOGGLE_URL  = 'https://github.com/fpc-unleashed/freepascal/releases/download/components-v1/ToggleDisplayAffinity.zip';
  COMPONENTS_TOGGLE_SHA  = '7EA739C994FD725FBD30EFBE216DD97732A64BC26D41EB53B03759441DB80E1E';
...
function c_setenv(name, value: PChar; overwrite: LongInt): LongInt; cdecl; external 'c' name 'setenv';
{$endif}

const
  // upper bound (on 0..100) of each stage's overall-progress slice. index 0
  // (isInit) is implicitly 0; subsequent values are the cap.
...
const
  // baked minimal Lazarus environmentoptions.xml. Version 112 / Lazarus 4.99
  // matches the current main of fpc-unleashed/lazarus. ActiveDesktop="default
  // docked" makes the IDE start in single-window dock layout
  // (anchordockingdsgn handles the runtime when the package is installed --
  // which we do via lazbuild --add-package). InitialFPCSrcRescanDone skips
  // the slow first-launch scan over <fpcsrc>.
  ENV_OPTIONS_TEMPLATE: string =
    ...
    '  </EnvironmentOptions>'#13#10+
    // explicit Desktop1 + Desktop2 so the IDE has named entries to activate;
    // DockMaster ties Desktop2 to anchordocking. layout details (window
    // positions etc.) are filled in on first save.
  ...
  // pre-acknowledge the "Enable anchor docking?" prompt. without this the IDE
  // shows a blocking dialog on first run.
  ...
  // FpDebug is the user fork's preferred backend (modern, internal, no external
  // gdb.exe required). without this file the IDE pops the "Configure Lazarus
  // IDE" wizard on first launch with a red "!" on the Debugger tab. UID is a
  // fixed GUID matching unleashed21's reference install.
...
begin
  FStage := s;
  // entering stage: park bar at this stage's start (= prev stage end); status
  // text shows the stage name only until sub-progress arrives
  Progress(0, '...');
end;

// installer.log lives in the install dir alongside installer.ini so a user
// inspecting the install has everything in one place. truncated each run
// (fmCreate in Execute).
...
// pull "[ NN%]" out of a lazbuild --build-ide line (or any line that uses that
// progress format) and feed it into the current stage's progress bar. returns
// False if no usable percent was found.
...
begin
  // make + fpc are loud. drop the two lowest-severity diagnostics (Hint and
  // Note) to keep the user-visible log readable; Warning and above still come
  // through, plus structural lines (Compiling, Linking, make[n]: Entering, ...).
  if Pos('Hint:', Line) > 0 then Exit;
  if Pos('Note:', Line) > 0 then Exit;
  Log(Line);
  // lazbuild's --build-ide emits "[ NN%] ..." every package; map that straight
  // into the current stage's progress slice
  var Pct: Integer;
...
begin
  // FPC source tree lives at <install>\fpcsrc - flat, sibling of fpc/ and
  // lazarus/. Both make targets and the lazarus IDE config point at this path.
...
// Linux-only: scan <install>/fpc/lib/fpc/ for a version-like subdir (e.g.
// "3.2.2", "3.3.1") that contains a ppcx64 binary. fpc-unleashed is currently
// 3.3.1 while the bootstrap is 3.2.2; we can't hardcode the version because
// the source tree decides it (FPC_VERSION in fpcdefs.inc). Result cached
// after first successful detection.
...
{$ifdef LINUX}
  ...
  if DirectoryExists(Base) then begin
    ...
    try
      repeat
        if (SR.Name <> '.') and (SR.Name <> '..') and ((SR.Attr and faDirectory) <> 0) and (Length(SR.Name) > 0) and (SR.Name[1] in ['0'..'9']) and FileExists(IncludeTrailingPathDelimiter(Base)+SR.Name+'/ppcx64') then begin
...
{$endif}
  // Pre-install fallback: matches the bootstrap so paths log sensibly until
  // make install lands the real version on disk. Real callers (HostFpcBinDir
  // et al.) invoke us after make install, so they get the detected value.
  if FHostFpcVersion = '' then Result := '3.2.2'
...
{$endif}
{$ifdef LINUX}
  // Layout is <install>/fpc/lib/fpc/<detected-version>/. Version is probed at
  // runtime by HostFpcVersion -- can't hardcode because the freshly-built
  // unleashed compiler reports its own version (3.3.1+), not the bootstrap's (3.2.2).
...
// Used by the dispatcher's hasCross<arch> detection and by the manifest writer
// to decide what cross targets are physically installed.
function TInstallThread.HostFpcUnitsDir: string;
begin
{$ifdef MSWINDOWS}
  Result := IncludeTrailingPathDelimiter(IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc')+'units'+PathDelim;
{$endif}
{$ifdef LINUX}
...
// Shell wrapper installed AS bin/fpc that sets PPC_CONFIG_PATH then exec's the
// original launcher (moved to bin/fpc.real). FPC reads ~/.fpc.cfg first if
// present, so without this a stale user cfg shadows our portable one for
// Lazarus and shell invocations. Idempotent (shebang check). No-op on Windows.
...
{$ifdef LINUX}
  ...
  // write wrapper. SCRIPT_DIR via $(dirname $(readlink -f $0)) so the wrapper
  // still finds fpc.real if the install dir gets moved.
  var Wrapper :=
    '#!/bin/sh'#10+
...
// After `make install` on Linux, ppcx64 / ppcross* end up in lib/fpc/<ver>/ but
// no symlinks land in bin/. The `fpc` launcher looks for `ppc<target>` in its
// own bin/ first; without the symlink fpc exits with "ppcx64 can't be executed,
// code 127" (POSIX ENOENT from exec). Distro packaging normally drops these
// symlinks but the upstream Makefile doesn't, so we create them ourselves.
//
// Relative target so the install dir stays movable. No-op on Windows (everything
// sits flat in bin\x86_64-win64\ already).
...
// recursive directory removal -- ignores missing target. On Windows shells
// `cmd /C rmdir /S /Q`; on Linux invokes `/bin/rm -rf`.
...
{$ifdef LINUX}
  // ZIP does not preserve unix exec bits (TUnZipper drops them); restore +x on
  // every file under bin/ (fpc wrapper, fpcmkcfg, fpcres, ...) and on the
  // compiler binary in lib/fpc/3.2.2/. -R + glob covers all the bin/ utilities
  // in one shot.
...
{$endif}
  ...
  Log('Bootstrap ready: '+IncludeTrailingPathDelimiter(BootstrapBinDir)+BootstrapPpName+ExeExt);
  Result := True;
end;

// after extract, codeload leaves a single top-level dir like "freepascal-abc123def..."
// containing the actual source. find that one directory in ParentDir; return ''
// if not exactly one dir there.
...
begin
  ...
  // hidden temp parent so FindOnlyTopDir works regardless of siblings (fpc,
  // fpc322, lazarus, ...) already living in TargetDir
...
// run make with the given args from the source dir. On Windows the bootstrap
// zip ships its own make.exe + binutils (as.exe, ld.exe, ar.exe), so we point
// make at the bootstrap copy and prepend the bootstrap bin dir to PATH. On
// Linux we use the system `make` (build-essential) from PATH and rely on
// system binutils (also in PATH already); the FPC linux bootstrap zip does
// not bundle GNU make.
...
{$ifdef LINUX}
  ...
  // FPC Makefile (~lines 105-140) runs `$(FPC) -iVSPTPSOTO` to detect the
  // compiler. If $(FPC) resolves empty, FULL_TARGET becomes '-' and
  // Makefile:252 errors "doesn't support target -". Prepend both bootstrap
  // bin/ (launcher) and lib/fpc/3.2.2/ (actual ppcx64) to PATH so detection
  // succeeds. On Windows the i386-win32 bootstrap ships fpc.exe and make.exe
  // in the same dir, already covered by BootstrapBinDir.
  var BootstrapBinUnix := IncludeTrailingPathDelimiter(IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc322')+'bin';
  var PathPrefix := BootstrapBinUnix+PathSeparator+BootstrapBinDir;
  // diagnostic: one-shot dump of which make we'll exec + what's in parent env
  // that could affect it. helps trace cases where the distro ships a make
  // wrapper (ccache, etc.) or leaves MAKEFLAGS hanging in the user's shell.
...
begin
  Result := False;
  // On Windows the bootstrap PP is ppc386.exe (FPC 3.2.2 i386 native); it
  // cross-builds the host x86_64 compiler. The diagnostic scripts proved this
  // path end-to-end; switching to a x86_64-targeting bootstrap broke `make all`
  // for the native step (system.inc could not find x86_64.inc). On Linux the
  // bootstrap is the upstream x86_64-linux portable tarball; PP is the native ppcx64.
  ...
  var PpSelf           := IncludeTrailingPathDelimiter(WorkDir)+'compiler'+DirectorySeparator+'ppcx64'+ExeExt;
  var FpcInstallPrefix := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc';

  // Win64 lacks native 80-bit Extended so the native ppcx64.exe must be built
  // with the same `-dFPC_SOFT_FPUX80` as the cross compilers. OS-only cross
  // (same CPU, different OS) keeps ppcx64.exe per compiler/utils/fpc.pp:480-484,
  // so a soft-x80 cross-rtl loaded by a native compiler without the flag IEs
  // 200208151 on first .ppu read. Linux x86_64 has native x87 Extended and
  // needs no flag.
{$ifdef MSWINDOWS}
...
{$endif}
  ...
  if not RunMake(['all', 'OS_TARGET='+HostTargetOs, 'CPU_TARGET=x86_64', 'PP='+PpBootstrap]+SoftX80, 'make all (native FPC, ~5-10 min)') then Exit;

  SetStage(isFpcMakeUtils);
  if not RunMake(['utils', 'OS_TARGET='+HostTargetOs, 'CPU_TARGET=x86_64', 'PP='+PpSelf]+SoftX80, 'make utils') then Exit;

  SetStage(isFpcMakeInstall);
  if not RunMake(['install', 'OS_TARGET='+HostTargetOs, 'CPU_TARGET=x86_64', 'INSTALL_PREFIX='+FpcInstallPrefix, 'PP='+PpSelf]+SoftX80, 'make install') then Exit;

  // On Linux, `make install` doesn't drop bin/ppc* symlinks; the distro
  // packaging usually does. Without them `fpc` launcher exits with code 127
  // because it can't find ppc<target> in its own dir. No-op on Windows (flat
  // bin\<target>\ layout already has ppcXXX).
  EnsureCompilerSymlinks;
  // Wrap bin/fpc so that ANY invocation (shell, Lazarus IDE, makefiles) gets
  // PPC_CONFIG_PATH set automatically. Defends against stale ~/.fpc.cfg
  // without needing user action. No-op on Windows (config-search order there
  // favors the compiler-relative fpc.cfg already).
  InstallFpcWrapper;
...
begin
  ...
  // -dFPC_SOFT_FPUX80 is mandatory here because the target is i386 and the
  // host (x86_64-win64) does not define FPC_HAS_TYPE_EXTENDED. The hard gate
  // at fpcdefs.inc:432 ('Cross-compiling from systems without support for an
  // 80 bit extended floating point type to i386 is not yet supported') fires
  // otherwise.
  if not RunMake(['crossinstall', 'OS_TARGET=win32', 'CPU_TARGET=i386', 'INSTALL_PREFIX='+FpcInstallPrefix, 'PP='+PpSelf, 'OPT=-dFPC_SOFT_FPUX80'], 'make crossinstall (i386-win32, ~5 min)') then Exit;
...
begin
  ...
  // FPC has an internal WASM linker; no external binutils or libc to bundle.
  // wasm bytecode is its own format (not ELF), so the crossinstall produces
  // only ppcrosswasm32 + RTL units. OS target is "wasip1" (WASI Preview 1);
  // the older "wasi" alias is not understood by the current Makefile.
  if not RunMake(['crossinstall', 'OS_TARGET=wasip1', 'CPU_TARGET=wasm32', 'INSTALL_PREFIX='+FpcInstallPrefix, 'PP='+PpSelf], 'make crossinstall (wasm32-wasip1, ~2 min)') then Exit;
...
begin
  ...
  var UnitsDir   := IncludeTrailingPathDelimiter(FpcInstall)+'units'+DirectorySeparator+'wasm32-wasip1';
...
// download a zip to DestZip and verify its SHA256 matches the pinned Sha. Body
// of the network IO + hash check shared between cross-Linux 64/32 (each pulls
// two zips: BIN binutils + LIB glibc/runtime).
...
// add or remove a per-target cross-compile section in fpc.cfg. Each section is
// wrapped in tagged BEGIN/END comments so re-installs can strip the previous
// version cleanly without accumulating duplicates. Add=False simply removes any
// existing tagged block (used by StepRemoveCrossLinux*).
...
// CPU_SOURCE/OS_SOURCE/FPCDIR/FPCFPMAKE keeps make from inferring the host
// triple from PP -- without them, an i386 PP makes make think the source
// platform is i386, which breaks the host fpmkunit bootstrap during packages_all.
...
// Shared download+extract for both Linux cross zips. Returns False on any
// failure with FErrorMsg set; the caller logs context.
...
begin
  ...
  var CrossDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'cross'+DirectorySeparator+'x86_64-linux';
  ...
  var PpCrossBuilt := IncludeTrailingPathDelimiter(MakeWorkDir)+'compiler'+DirectorySeparator+'ppcrossx64'+ExeExt;
...
begin
  Result := True;  // best-effort
  var CrossDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'cross'+DirectorySeparator+'x86_64-linux';
  var UnitsDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc'+DirectorySeparator+'units'+DirectorySeparator+'x86_64-linux';
...
begin
  ...
  // it supports both -Twin32 and -Tlinux at runtime, so the same binary serves
  // both targets. Without it, we cannot produce a writer for the cross-RTL
  // whose .ppu the same binary will later read.
  ...
  var CrossDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'cross'+DirectorySeparator+'i386-linux';
  var BinDir   := IncludeTrailingPathDelimiter(CrossDir)+'bin';
  var LibDir   := IncludeTrailingPathDelimiter(CrossDir)+'lib';
  ...
  // OS_TARGET=linux re-stages host RTL with an i386 compiler against win64
  // source paths and fails on 'Cannot open i386.inc'.
  //
  // A prior `make distclean` (native or earlier cross step) wiped
  // compiler/msgtxt.inc + msgidx.inc; rtl_all needs them via verbose.pas ->
  // cmsgs.pas. Run the `msg` target with the host compiler to regenerate --
  // cheap, leaves everything else alone.
  ...
  // stages 2-3: rtl_all + rtl_install, FPC=ppcross386, no OPT (ppcross386 already has soft-x80 baked in from build_win32 step)
...
begin
  Result := True;  // best-effort
  var CrossDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'cross'+DirectorySeparator+'i386-linux';
  var UnitsDir := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc'+DirectorySeparator+'units'+DirectorySeparator+'i386-linux';
...
// Cross win64 from linux host using FPC's internal linker (-Xi); no mingw-w64
// binutils needed. Six-stage build mirrors the linux64-from-win64 case
// (compiler_cycle/install with host PP, rtl_*/packages_* with the just-built
// ppcrossx64) for the same .ppu-compatibility reason.
...
begin
  ...
  var PpCrossInstalled := HostFpcBinDir+'ppcrossx64'+ExeExt;
  var FpcInstallPrefix := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc';
  // Common args: NO BINUTILSPREFIX / CROSSBINDIR / CROSSOPT -- FPC's internal
  // linker handles PE/COFF without external `as`/`ld`. Pass -Xi via OPT so
  // the compiler is explicit about wanting the internal path even if a system
  // mingw-w64 happens to be in PATH.
  ...
  if not RunMake(['compiler_cycle', 'FPC='+PpHost]+Common, 'compiler_cycle (x86_64-win64, ~3 min)') then Exit;
  ...
  if not RunMake(['compiler_install', 'FPC='+PpHost]+Common, 'compiler_install (x86_64-win64)') then Exit;

  var PpForRtl := if FileExists(PpCrossInstalled) then PpCrossInstalled else PpCrossBuilt;
  Log('  using cross compiler for RTL/packages: '+PpForRtl);

  // stages 3-4: rtl_all + rtl_install -- run with the cross compiler so the
  // .ppu it writes are readable by it later (writer/reader match).
  Log('  stage 3/6: rtl_all (RTL via ppcrossx64)');
  ...
  // No fpc.cfg patch needed for win64-from-linux cross: -Xi is enabled by the
  // OPT we pass at build time, and user-side compiles via `fpc -Twin64` rely
  // on the same internal linker. If we later add a mingw-w64 binutils
  // fallback path, it would go here as a cfg block.

  // refresh bin/ symlinks so the newly-built ppcrossx64 is findable through
  // the `fpc` launcher (no-op on Windows).
  EnsureCompilerSymlinks;
...
// Build the i386-win32 cross compiler from a x86_64-linux host using the FPC
// internal PE/COFF linker (-Xi). Same staged pattern as the
// x86_64-win64-from-linux step: compiler_cycle to produce ppcross386, then
// rtl + packages with that compiler as FPC=.
//
// No -dFPC_SOFT_FPUX80 here: source host x86_64-linux has native extended via
// System V ABI, target i386 has native x80 in hardware, so neither end of the
// writer/reader pair fakes Extended; the .ppu stays consistent.
...
begin
  ...
  if not RunMake(['compiler_cycle', 'FPC='+PpHost]+Common, 'compiler_cycle (i386-win32, ~3 min)') then Exit;

  var PpCrossBuilt := IncludeTrailingPathDelimiter(MakeWorkDir)+'compiler'+DirectorySeparator+'ppcross386'+ExeExt;
  if not FileExists(PpCrossBuilt) then begin
    FErrorMsg := 'compiler_cycle did not produce '+PpCrossBuilt;
    Log('  '+FErrorMsg);
    Exit;
  ...
  Log('  stage 2/6: compiler_install');
  if not RunMake(['compiler_install', 'FPC='+PpHost]+Common, 'compiler_install (i386-win32)') then Exit;
...
// ppcross386 (produced by StepBuildFpcCrossWin32FromLinux) since the same
// i386 codegen binary serves both -Twin32 and -Tlinux at runtime. Skips
// compiler_cycle: rebuilding ppcross386 for OS_TARGET=linux would re-stage
// host RTL with an i386 compiler against x86_64-linux source paths and choke.
// Just regenerate msg includes (compiler/msgtxt.inc + msgidx.inc that
// distclean wiped) and proceed straight to rtl_all using the existing
// ppcross386 -- same shortcut as the Win-host linux32 step (StepBuildFpcCrossLinux32).
function TInstallThread.StepBuildFpcCrossLinux32FromLinux: Boolean;
begin
  Result := False;
  var Pp := HostFpcBinDir+'ppcross386'+ExeExt;
  if not FileExists(Pp) then begin
    FErrorMsg := 'i386-linux cross requires the i386-win32 cross compiler. Tick "i386-win32" in the cross list as well, then run install again.';
    Log('  '+FErrorMsg);
    Exit;
  end;
...
begin
  ...
  // We deliberately don't remove ppcross386 here -- it may still be in use by
  // the i386-win32 target on this host.
end;
...
begin
  ...
  // a hidden temp parent so FindOnlyTopDir works regardless of what else sits
  // next to the install dir (fpc, fpc322, src, ...)
...
const
  // Packages installed into the IDE statically. Base packages first reduces
  // dep churn (lazbuild resolves but is faster on a clean queue). Paths
  // relative to <lazarus>\.
  ...
  // Docked IDE. anchordocking added as link only; its dsgn package pulls it
  // in for IDE static linkage.
  ...
  // Optional CPU-View add-on lives outside the lazarus checkout to keep the
  // lazarus repo small (sources are downloaded on demand from the components-v1
  // release into <install>/components-extra/). FWHexView ships its design-time
  // .lpk under a single name on all platforms; CPUView has a per-platform .lpk
  // because the runtime pulls in platform-specific debugger glue (Windows API
  // vs ptrace/lldb). Paths here are relative to <TargetDir>; AddPackage prepends
  // LazarusDir which is wrong for these -- the wired-up loop below uses absolute
  ...
  // Host-platform-specific CPUView design-time .lpk filename. Runtime pieces
  // live inside the same .lpk -- there is no separate runtime/design split.
  // Selection is forced at the host level here because the .lpk explicitly
  // carries the target triple in its name.
{$ifdef MSWINDOWS}
...
{$endif}
  ...
  // ToggleDisplayAffinity ships a single .lpk at the zip root (no per-platform
  // variants). The const exists on every host so callers can reference it
  // without ifdef, but the only caller that touches disk is inside a
  // {$ifdef MSWINDOWS} block.
...
{$endif}
  ...
  // every lazbuild invocation gets the same boilerplate so package and IDE
  // builds agree on pcp/cpu/os/lazarusdir
...
// Same as AddPackage but for packages outside LazarusDir (the optional add-ons
// under <TargetDir>/components-extra/). lazbuild itself does not care where
// the .lpk lives, only that it can resolve required packages -- which it does
// via the per-user pcp's known-package list, populated by `--add-package` /
// `--add-package-link` itself.
function TInstallThread.AddPackageAbs(const LpkAbs: string; LinkOnly: Boolean): Boolean;
begin
  var Mode := if LinkOnly then '--add-package-link' else '--add-package';
  Result := RunLazbuild([Mode, LpkAbs], Mode+' '+ExtractFileName(LpkAbs));
end;

// Register the CPU-View triple: FWHexView.LCL (runtime dep, link-only so it
// does not end up statically linked into the IDE -- the design-time .lpk does),
// FWHexView_D.LCL (design-time, IDE component), and the host-platform
// CPUView_<plat>_D.lpk (design-time, IDE plugin). Order matters: lazbuild
// resolves RequiredPkgs against the known-packages list, so the runtime dep
// must be registered before its dependents.
function TInstallThread.RegisterCPUViewPackages: Boolean;

  // The COMPONENTS_*_LPK constants are authored with backslashes so the strings
  // look natural on Windows. On Linux backslash is a legal filename character
  // (FileExists treats 'comp\foo' as a literal name, not a directory traversal),
  // so we must normalize to the host's separator before any disk lookup. lazbuild
  // itself is more forgiving -- it parses .lpk paths through its own canonicalizer
  // -- but our pre-flight FileExists check sits in front of lazbuild and needs
...
begin
  ...
  if not FileExists(FwhexRt) then begin
    FErrorMsg := 'CPU-View addon: missing '+FwhexRt+' (was StepDownloadComponents skipped?)';
    Log('  '+FErrorMsg);
    Exit;
  end;
  ...
  if not FileExists(Cpuview) then begin
    FErrorMsg := 'CPU-View addon: missing '+Cpuview+' (no .lpk for this host platform?)';
    Log('  '+FErrorMsg);
    Exit;
  end;
...
// Tear down what RegisterCPUViewPackages wrote into the IDE config. Order is
// the reverse of registration: design-time first (because removing the runtime
// first would leave a dangling RequiredPkgs link in the design-time entries
// until the next IDE rebuild).
...
// Add a "CPU-View" entry to the IDE editor toolbar in <pcp>/environmentoptions.xml.
// The IDE stores the toolbar layout per-desktop inside
// <Desktops>/<Desktop{N}>/<EditorToolBarOptions>, with a numbered
// <Button{i} Name="..."/> sequence and a Count attribute. On a fresh install
// the IDE has not yet been launched, so the section does not exist -- silently
// skip in that case (the IDE will create the default toolbar on first run,
// and a subsequent installer re-run with CPU-View still selected will then
// inject the button).
...
begin
  ...
  // <Desktops> is a sibling of <EnvironmentOptions> under <CONFIG>, not a child
  // of it. The path must root at "Desktops/" -- using the
  // "EnvironmentOptions/Desktops/..." prefix silently returns 0 via TXMLConfig's
  // missing-path default and the function becomes a no-op.
  ...
  for var d := 1 to DesktopsCount do begin
    var Base := 'Desktops/Desktop'+IntToStr(d)+'/EditorToolBarOptions/';
    var Cnt: Integer := Cfg.GetValue(Base+'Count', 0);
    // section absent -> nothing to add; the IDE will write its default toolbar
    // block on first save and a later installer run can append the button then.
    if Cnt = 0 then Continue;

    var EmptyStr: string := '';
    var AlreadyThere: Boolean := False;
    for var i := 1 to Cnt do
      if Cfg.GetValue(Base+'Button'+IntToStr(i)+'/Name', EmptyStr) = ButtonName then begin
        AlreadyThere := True;
        Break;
      end;
    ...
    Touched := True;
    Log('  added CPU-View toolbar button to Desktop'+IntToStr(d)+' (now '+IntToStr(Cnt+1)+' buttons)');
  end;

  if Touched then Cfg.Flush;
end;

// Reverse of RegisterCPUViewToolbarButton: drop the CPU-View entry from every
// desktop's editor toolbar, shifting later buttons down by one and decrementing
// Count. No-op if the button isn't present.
...
begin
  ...
  var Cfg := autofree TXMLConfig.Create(nil);
  Cfg.Filename := XmlPath;
  // Path rooted at "Desktops/" -- see RegisterCPUViewToolbarButton above for
  // why the "EnvironmentOptions/" prefix is wrong here.
  var DesktopsCount: Integer := Cfg.GetValue('Desktops/Count', 0);
  if DesktopsCount = 0 then Exit;
...
{$ifdef MSWINDOWS}
  ...
  // native fpc.exe before bootstrap so lazbuild's PATH-based compiler detection
  // picks the x86_64 wrapper; bootstrap stays for make + binutils
  var PathPrefix := HostFpcBinDir+PathSeparator+BootstrapBinDir;
{$endif}
{$ifdef LINUX}
...
{$endif}
  ...
  // 1. build lazbuild + LCL + minimum prereqs that the upcoming --add-package
  //    calls will need to compile each package against.
  ...
  // anchordocking is a runtime package; the IDE statically links its *dsgn
  // variant which depends on the runtime. add the runtime as a link only so
  // it ends up in package list but not in staticpackages.inc.
...
{$ifdef MSWINDOWS}
  if FCfg.InstallToggleAffinity then begin
    ...
    var TogglePath := StringReplace(IncludeTrailingPathDelimiter(FCfg.TargetDir)+COMPONENTS_TOGGLE_LPK, '\', DirectorySeparator, [rfReplaceAll]);
    if not FileExists(TogglePath) then begin
      FErrorMsg := 'Toggle Display Affinity addon: missing '+TogglePath+' (was StepDownloadComponents skipped?)';
      Log('  '+FErrorMsg);
      Exit;
    end;
...
{$endif}
  ...
  //    -dKeepInstalledPackages keeps the package list around between rebuilds
  //    so a re-run of --build-ide does not silently drop them. We do not pass
  //    -dAddStaticPkgs here -- the lazarus packagesystem
  //    (ide/packages/idepackager/packagesystem.pas:2533) emits it automatically
  //    when assembling the build command, so adding it on the call line is
  //    redundant. lazbuild prints "[ NN%]" lines that OnMakeLine catches and
  //    feeds back into our progress.
  SetStage(isLazIde);
  if not RunLazbuild(['--build-ide=-dKeepInstalledPackages'], 'lazbuild --build-ide (~5 min)') then Exit;

  // No toolbar XML touch on the fresh install path: the compiled IDE already
  // carries CPU-View as part of its default editor toolbar (the Lazarus fork
  // wires it in when the package is registered), so writing anything here
  // would just be a race against the IDE's own first-close rewrite of
  // environmentoptions.xml. Toolbar sync stays scoped to the addon-delta path
  // inside StepRebuildLazarusForAddons, where the file already exists and the
  // user genuinely flipped the addon state.

  Log('--- Lazarus ready: '+IncludeTrailingPathDelimiter(LazarusDir)+'lazarus'+ExeExt+' ---');
  Result := True;
end;

// Edit Lazarus's miscellaneousoptions.xml + packagefiles.xml in-place to drop
// the named package's registration. After this and a `lazbuild --build-ide`,
// the resulting IDE binary no longer statically links the package and the IDE
// no longer lists it in known-packages.
...
begin
  ...
  // miscellaneousoptions.xml controls what gets statically linked into the
  // IDE on `lazbuild --build-ide`.
  RemoveIndexedItem(Pcp+'miscellaneousoptions.xml', 'MiscellaneousOptions/BuildLazarusOptions/StaticAutoInstallPackages/', 'Value');
  // packagefiles.xml is the IDE's known-packages list (used by Package menu,
  // Open Package... etc).
  RemoveIndexedItem(Pcp+'packagefiles.xml', 'UserPkgLinks/', 'Name/Value');
end;

// "Reinstall" with a flipped addon checkbox does not need to redo the whole
// Lazarus install; just (a) run lazbuild --add-package on the newly-ticked
// addons OR remove the registration of the unticked ones, and (b) re-run
// --build-ide so the resulting binary statically links the new package set.
...
begin
  ...
  if FCfg.InstallCPUView and (not Prev.InstallCPUView) then begin
    ...
    // sources may already be cached from a prior tick-then-untick run, but a
    // fresh tick still has to fetch them if components-extra/ was wiped
    // between runs. Hook the download here so re-installs don't require
    // running the full bootstrap path again.
    ...
    // (Prev.InstallCPUView=False but Lazarus is built and Prev is from a
    // manifest of an existing install), so environmentoptions.xml should
    // already carry EditorToolBarOptions blocks the IDE wrote on first launch
    // -- the helper picks them up and appends CPU-View.
    RegisterCPUViewToolbarButton;
  end else if (not FCfg.InstallCPUView) and Prev.InstallCPUView then begin
...
{$ifdef MSWINDOWS}
  // Toggle Display Affinity delta: same skeleton as CPU-View but a single-package
  // add/remove (no runtime dependency, no toolbar button -- the plugin installs
  // itself as a Window menu entry which the IDE picks up directly from the
  // registered design-time package).
  if FCfg.InstallToggleAffinity and (not Prev.InstallToggleAffinity) then begin
    ...
    var TogglePath := StringReplace(IncludeTrailingPathDelimiter(FCfg.TargetDir)+COMPONENTS_TOGGLE_LPK, '\', DirectorySeparator, [rfReplaceAll]);
    if not AddPackageAbs(TogglePath) then Exit;
  end else if (not FCfg.InstallToggleAffinity) and Prev.InstallToggleAffinity then begin
...
{$ifdef MSWINDOWS}
  ...
  // on Linux the FPC bootstrap zip has no make; the IDE picks up the system
  // /usr/bin/make from PATH at runtime if MakeFilename is empty -- actually
  // leaving the default 'make' in environmentoptions.xml is what Lazarus prefers.
{$endif}
{$ifdef LINUX}
  var MakePath := 'make';
{$endif}
  // FPC compiler path Lazarus uses for code-completion + project builds. On
  // Linux the `fpc` shell wrapper in <prefix>/bin/ is what users normally
  // point IDEs at; it dispatches to ppcx64.
  var FpcCompilerPath :=
{$ifdef MSWINDOWS}
...
begin
  ...
  // --pcp tells Lazarus to load our isolated config_lazarus instead of the
  // default per-user dir (%LOCALAPPDATA%\lazarus on Windows, ~/.lazarus on Linux).
  ...
  // rendered with a yellow background + bold black text
  Log('============================================================');
...
begin
  Result := False;
  // fpcmkcfg lives in <prefix>/bin/<host-target>/ on Windows and in <prefix>/bin/
  // on Linux (standard unix prefix layout).
  var FpcMkCfg := HostFpcUtilDir+'fpcmkcfg'+ExeExt;
  // fpc.cfg sits next to the compiler binary on both OSes so FPC's config
  // search order finds it without needing /etc/fpc.cfg or ~/.fpc.cfg fallbacks.
  var CfgPath  := HostFpcBinDir+'fpc.cfg';
  // fpcmkcfg template uses %basepath%/units/$fpctarget for unit search paths.
  // The right basepath depends on the install layout:
  //   Windows: <install>/fpc/{units,bin}/<target>/  -- basepath=<install>/fpc
  //   Linux:   <install>/fpc/lib/fpc/<ver>/units/<target>/  -- basepath=<install>/fpc/lib/fpc/<ver>
  // Using <install>/fpc on Linux makes the template resolve to <install>/fpc/units/<target>/
  // which does not exist -> "Can't find unit system" at first Lazarus compile.
{$ifdef MSWINDOWS}
...
{$endif}
  ...
  // make install does not generate fpc.cfg (the official Inno Setup installer
  // does it as a [Run] post-install step). Without fpc.cfg the compiler can
  // not find unit search paths beyond rtl, breaking every non-trivial build
  // (e.g. lazarus -> "Can't find unit db"). template uses %basepath% to
  // resolve -Fu/-Fl/-FD paths.
  ...
  if not FileExists(FpcMkCfg) then begin
    FErrorMsg := 'fpcmkcfg binary not found at '+FpcMkCfg+' (make install did not place it -- check `make utils_install`)';
    Log('  '+FErrorMsg);
    Exit;
  end;
  // Defensive: lib/fpc/<ver>/ should exist after rtl_install but double-check
  // so fpcmkcfg's -o doesn't fail on a missing parent.
  ForceDirectories(HostFpcBinDir);
  // RunStream captures fpcmkcfg's stderr (lines) through OnMakeLine so a
  // non-zero exit surfaces the real reason ("Could not open template file ...",
  // "Could not create output file ..." etc.) in the UI log instead of just "exit=1".
...
{$ifdef LINUX}
  // fpcmkcfg's default template uses -FD%basepath%/bin/$FPCTARGET for tool
  // lookup (fpcres, fpcsubst, ...). That layout is what Windows has --
  // bin/<target>/. On Linux the unix prefix convention puts utilities flat in
  // <prefix>/bin/, so the default -FD resolves to a non-existent dir and
  // Lazarus compiles fail with "Resource compiler fpcres not found". Append
  // a working -FD that points at the real utility dir; FPC scans all -FD
  // entries until it finds the requested utility, so leaving the broken one
  // in place is harmless and saves us a search-and-replace.
  try
  ...
  // FPC's config-file search prefers ~/.fpc.cfg over <compiler-dir>/fpc.cfg if
  // BOTH exist (compiler-relative is the last-resort fallback, not the first
  // hit). A user with any prior FPC experiment leaves a ~/.fpc.cfg behind,
  // and its stale -Fu paths win over the portable fpc.cfg we just generated
  // -> "Can't find unit system" on every subsequent compile. Override the
  // search by pointing PPC_CONFIG_PATH at our cfg dir; ApplyEnvWithPathPrefix
  // copies it into every child make / lazbuild.
  ...
  // Warn if the user has ~/.fpc.cfg; our pipeline is now safe (overridden via
  // env), but `fpc` invoked manually from a shell after install will still
  // hit it unless the user removes/renames it.
...
{$endif}
  // Dump the unit-search-path lines so we can visually verify the template
  // resolved %basepath% correctly. -Fu lines that don't resolve to real dirs
  // on disk are how we end up with "Can't find unit system" at first compile.
  try
    var Sl := autofree TStringList.Create;
...
// Pull the optional add-on packages (FWHexView + CPUView) off the components-v1
// release into <TargetDir>/components-extra/. We never re-download: presence
// of the unzipped folder is the cache key. Failure here is fatal because the
// caller already decided the user wants the add-on -- proceeding without it
// would silently produce an IDE without the requested feature.
function TInstallThread.StepDownloadComponents: Boolean;
  ...
  begin
    Result := False;
    var ZipFile := IncludeTrailingPathDelimiter(GetTempDir)+Label_+'.zip';
...
begin
  Result := True;
  // Skip the whole stage if none of the optional component-bundle addons are
  // requested -- no need to touch the network at all. ToggleDisplayAffinity
  // only triggers the fetch on Windows hosts (its checkbox is locked off
  // elsewhere), so the guard reflects that.
...
{$ifdef MSWINDOWS}
  if FCfg.InstallToggleAffinity then begin
    ...
    if not FileExists(IncludeTrailingPathDelimiter(ToggleDir)+'toggledisplayaffinity.lpk') then begin
...
begin
  ...
  var FpcInstall := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc';
  var PpcrossBin := HostFpcBinDir+'ppcross386'+ExeExt;
  var UnitsDir   := IncludeTrailingPathDelimiter(FpcInstall)+'units'+DirectorySeparator+'i386-win32';
...
begin
  // crossinstall drops a 32-bit native bin tree alongside the cross bits; we
  // only want the cross compiler in x86_64-win64\, so ditch the 32-bit native
  // bin. Matches the workflow's "Cleanup" step. (fpc322 bootstrap stays --
  // lazarus build still needs make.exe and binutils from there.) i386-win32
  // cross is only meaningful on a Windows host (Linux host would
  // cross-from-linux which is a different code path entirely).
  var P := IncludeTrailingPathDelimiter(FCfg.TargetDir)+'fpc'+DirectorySeparator+'bin'+DirectorySeparator+'i386-win32';
...
begin
  ...
  try
    ...
    // open installer.log AFTER the target dir exists so the create call never
    // fails on a missing parent. fmCreate truncates each run - each install
    // gets its own clean log, no accumulation.
    ...
    // pipeline is idempotent: each step checks its end-state and skips if
    // already done. clicking 'Reinstall' on an existing install does not redo
    // a 25-minute build pointlessly; it just applies whatever delta the user
    // requested via checkboxes (typically: add or remove a cross compiler),
    // or - if the user picked a different commit/branch - refreshes only the
    // affected component.
    ...
    // Linux cross compilers don't get a dedicated ppcross<arch> binary (ppcx64
    // / ppcross386 are multi-OS), so detect by RTL units presence. x86_64-linux
    // units only signal "cross is installed" on Windows host; on Linux host
    // that dir IS the native target -- ditto for win64.
{$ifdef MSWINDOWS}
...
{$endif}
    ...
    var hasBootstrap    := FileExists(IncludeTrailingPathDelimiter(BootstrapBinDir)+BootstrapPpName+ExeExt);
    Log(Format('current state: fpc=%s laz=%s cross386=%s wasm=%s linux64=%s linux32=%s win64=%s bootstrap=%s',
      [BoolToStr(hasFpcExe, True), BoolToStr(hasLazExe, True), BoolToStr(hasCrossW32, True), BoolToStr(hasCrossWasm, True),
       BoolToStr(hasCrossLinux64, True), BoolToStr(hasCrossLinux32, True), BoolToStr(hasCrossWin64, True), BoolToStr(hasBootstrap, True)]));

    // i386-linux needs ppcross386 (the same i386 codegen binary serves both
    // -Twin32 and -Tlinux at runtime; it's produced by the i386-win32 cross
    // step). If the user requested linux32 without already having ppcross386
    // AND without ticking i386-win32 in the same run, fail upfront with a
    // clear message rather than running 10 minutes of native build only to
    // crash at stage 1/5 of linux32.
    ...
    // compare manifest SHAs against what the UI selected; if they differ,
    // force a refresh of just that component.
...
{$endif}
    ...
    if hasBootstrap then Log('bootstrap fpc322 already installed, skipping')
      ...
      // bootstrap is needed for any make-based build below; only run if we
      // will actually need it (FPC build or cross compiler add)
      if (not hasFpcExe) or (FCfg.CrossWin32 and not hasCrossW32) or (FCfg.CrossWasm and not hasCrossWasm) or (FCfg.CrossLinux64 and not hasCrossLinux64) or (FCfg.CrossLinux32 and not hasCrossLinux32) or (FCfg.CrossWin64 and not hasCrossWin64) then
        if not StepBootstrap then Exit;
    end;

    // FPC source + native build - skip if FPC binary already there. user must
    // manually wipe <fpc> to force a rebuild.
    ...
    // fpc.cfg has to exist before any cross step that wants to patch it (Linux
    // cross appends a target-specific section). Pulled ahead of the cross
    // blocks so a brand-new install lands in the right order. Always
    // regenerate: cheap (~50ms) and avoids the "stale fpc.cfg from a previous
    // installer version with wrong basepath" trap.
    SetStage(isFpcCfg);
...
{$endif}
    ...
    // cross wasm32-wasip1: same smart add/remove pattern. WASM has no external
    // binutils / libc, so the build is just compiler + RTL units.
...
{$ifdef LINUX}
    // cross x86_64-win64 from a linux host: uses FPC's internal PE/COFF linker
    // (-Xi), so no external mingw-w64 binutils / Win64 import libs to ship.
    // Same idempotent add/remove pattern as the other crosses.
    ...
    end else if (not FCfg.CrossWin64) and hasCrossWin64 then begin
      if not StepRemoveCrossWin64FromLinux then Exit;
    end else if hasCrossWin64 then Log('cross compiler x86_64-win64 already installed, leaving as is')
...
{$ifdef MSWINDOWS}
    if FCfg.CrossLinux64 and (not hasCrossLinux64) then begin
      ...
      var BinDir := TargetPrefix+'cross'+DirectorySeparator+'x86_64-linux'+DirectorySeparator+'bin';
      var LibDir := TargetPrefix+'cross'+DirectorySeparator+'x86_64-linux'+DirectorySeparator+'lib';
...
{$endif}
    ...
    // cross i386-linux: requires ppcross386 from the i386-win32 step (same
    // multi-target binary serves both -Twin32 and -Tlinux).
{$ifdef MSWINDOWS}
    if FCfg.CrossLinux32 and (not hasCrossLinux32) then begin
      ...
      var BinDir := TargetPrefix+'cross'+DirectorySeparator+'i386-linux'+DirectorySeparator+'bin';
      var LibDir := TargetPrefix+'cross'+DirectorySeparator+'i386-linux'+DirectorySeparator+'lib';
...
{$endif}
{$ifdef LINUX}
    // Linux host -> i386-linux: requires ppcross386 (built either here in the
    // i386-win32 step that just ran, or already present from a prior run).
    // Internal -Xi for ELF generation; no external i386 binutils bundle needed.
    if FCfg.CrossLinux32 and (not hasCrossLinux32) then begin
      SetStage(isFpcCrossLinux32);
      if not StepBuildFpcCrossLinux32FromLinux then Exit;
    end else if (not FCfg.CrossLinux32) and hasCrossLinux32 then begin
      if not StepRemoveCrossLinux32FromLinux then Exit;
    end else if hasCrossLinux32 then Log('cross compiler i386-linux already installed, leaving as is')
...
{$endif}
    ...
    if FCfg.InstallLazarus and (not hasLazExe) then begin
      ...
      // download CPU-View / FWHexView before lazbuild --add-package runs so
      // the package-registration loop in StepBuildLazarus finds the .lpk files
      // on disk. Stage progress moves between make-lazbuild and package registration.
      SetStage(isLazComponents);
      if not StepDownloadComponents then Exit;
      ...
      // Lazarus is already built. Check whether the user changed any addon
      // selection vs what the manifest recorded last time -- if so, run a
      // smaller "add packages + rebuild IDE" step instead of a full reinstall.
      var addonsChanged := (FCfg.InstallMinimap <> Manifest.InstallMinimap) or (FCfg.InstallCPUView <> Manifest.InstallCPUView)
{$ifdef MSWINDOWS}
...
{$endif}
    ...
    // Cross-target detection by RTL units presence (the multi-target ppcrossx64
    // / ppcross386 binaries are reused, so units dir is the signal). On
    // Windows host x86_64-win64 is native (always present) and CrossWin64 is
    // meaningless -> record user's intent. On Linux host x86_64-linux is
    // native -> CrossLinux64 mirrors intent.
{$ifdef MSWINDOWS}
...
{$endif}
    ...
    // ToggleDisplayAffinity is Windows-only. On Linux, FCfg's value is always
    // False (UI checkbox locked off), so writing it here would erase a flag a
    // previous Windows install set. Preserve whatever ReadManifest returned
